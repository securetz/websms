...
class WebsmsSMSAPI
{
     ...
     */
    private function make_sms_text($sms_text){
        $post_fields = '';
        foreach ($sms_text as $key => $value) {
     ...
     */
    public function send_sms($sms_text, $url){
        $post_body='action=send-sms&'.$this->make_sms_text($sms_text,$url);
